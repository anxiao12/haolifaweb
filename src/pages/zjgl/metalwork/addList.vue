    ...
    <div>
        <div class="flex-v-center tool-bar">
            ...
            >
                ...
                <i
                    class="icon"
                    style="margin-left: -20px;margin-right:10px;pointer-events:none;"
                >arrow_drop_down</i>
                质检状态：
                <select
                ...
                >
                    <option
                        v-for="item in inspectStatusArr"
                        :value="item.status"
                        v-bind:key="item.id"
                    >{{item.name}}</option>
                </select>
            ...
            >
                ...
                >
                    ...
                    <td class="t-right">
                        ...
                        <a
                            href="javascript:;"
                            v-if="item.inspectStatus != 2"
                            style="margin-right: 3px"
                            class="blue"
                            @click="addInspectHistory(item)"
                        >添加质检记录</a>
                        <a
                            href="javascript:;"
                            v-if="item.inspectStatus != 2"
                            style="margin-right: 3px"
                            class="blue"
                            @click="updateEntrustStatus(item)"
                        >质检完成</a>
                    </td>
                </template>
            </data-list>
        ...
        >
            ...
            >
                ...
                <div class="flex">
                    <input-box
                        :disabled="true"
                        v-model="inspectHistoryAdd.purchaseNo"
                        class="flex-item mr-20 ml-20"
                        label="采购合同号"
                    ></input-box>
                    <input-box
                        :disabled="true"
                        v-model="inspectHistoryAdd.batchNumber"
                ...
                <div class="flex mt-15">
                    <input-box
                        :disabled="true"
                        class="flex-item mt-15 ml-20"
                        v-model="inspectHistoryAdd.materialGraphNo"
                        label="物料图号"
                    ></input-box>
                    <input-box
                        :disabled="true"
                        v-model="inspectHistoryAdd.materialName"
                        class="flex-item mr-20 mt-15 ml-20"
                        label="物料名称"
                    ></input-box>
                </div>
                <div class="flex mt-15">
                    <input-box
                        ...
                        class="flex-item mr-20 ml-20 mt-15"
                        label="检测数量"
                    ></input-box>
                    <input-box
                        v-model="inspectHistoryAdd.qualifiedNumber"
                        class="flex-item mr-20 mt-15"
                        label="合格数量"
                    ></input-box>
                    <input-box
...
export default {
    ...
    methods: {
        ...
        updateTask(orderNo) {
            ...
            this.$http
                .get(`/haolifa/pay-working-procedure/assignTaskInspect?type=3&orderNo=` + orderNo)
                .then((res) => {
        ...
        saveSelect() {
            ...
            this.$http
                .post(`/haolifa/pay-working-procedure/saveEntrustTask`, this.form)
                .then((res) => {
