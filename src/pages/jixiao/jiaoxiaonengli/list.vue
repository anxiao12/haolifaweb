    ...
    <div class="page-jiaoxiaogongxu-list">
        ...
        <div class="flex-v-center tool-bar">
            ...
            $refs.list.update(true)" placeholder="部门名称" style="width: 200px;">
                <input type="text" class="flex-item" v-model="filter.capacityCode" @change="
            $refs.list.update(true)" placeholder="能力代码" style="width: 200px;">
            </div>
            <div class="flex-item"></div>
            <btn class="b" flat color="#008eff" @click="add">新增</btn>
        </div>
        <div class="flex-item scroll-y">
            <data-list ref="list" :page-size="15" :param="filter" url="/haolifa/pay-production_capacity/getList" method="post">
                <tr slot="header">
        ...
        <layer v-if="layer" :title="form.id ?'编辑':'新增' " width="50%">
            <div class="layer-text" style="padding-bottom: 50px;">
                <div class="flex">
                    <el-select class="flex-item mr-10 mycs" v-model="form.departId" filterable placeholder="部门选择" @change="deptChange">
                        <el-option v-for="(obj,j) in deptList" :key="j" :label="obj.deptName" :value="obj.id"></el-option>
                    </el-select>
                </div>
                <div class="flex">
                    <el-select class="flex-item mr-10 mycs" v-model="form.userId" filterable placeholder="人员选择">
                        <el-option v-for="(obj,j) in userList" :key="j" :label="obj.userName" :value="obj.id"></el-option>
                    </el-select>
                </div>
                <div class="flex">
                    <el-select class="flex-item mr-10 mycs" v-model="capacityCode" :multiple="true" filterable placeholder="能力序号">
                        <el-option v-for="(obj,j) in procedureList" :key="j" :label="obj.text" :value="obj.value"></el-option>
                    </el-select>
                </div>
            </div>
    ...
    methods: {
        ...
        save() {
            ...
            let url = this.form.id
                ? "/haolifa/pay-production_capacity/edit"
                : "/haolifa/pay-production_capacity/save";
            this.$http
                .post(url, this.form)
                .then(res => {
                    this.close();
