    ...
    <div class="page-invoice-list">
        ...
        <div class="flex-v-center tool-bar">
            ...
            >
                ...
                <input
                    type="text"
                    class="flex-item"
                    v-model="filter.appSpecifications"
                    @change="
                $refs.list.update(true)"
                    placeholder="适用规格"
                    style="width: 180px;"
        ...
        <div class="flex-item scroll-y">
            <data-list
                ...
                :page-size="15"
                :param="filter"
                url="/haolifa/pay-hour_quota/getList"
                method="post"
            >
                <tr slot="header">
        ...
        >
            ...
            >
                ...
                <div class="flex">
                    ...
                    ></input-box>
                    <input-box
                        v-model="form.appSpecifications"
                        class="flex-item mr-20"
                        label="适用规格"
                    ></input-box>
                </div>
...
export default {
    ...
    methods: {
        ...
        remove(item) {
            this.$confirm({
                title: "删除确认",
                text: `您确定要删除以下工序名称吗？<br><b>${item.procedureName}</b>`,
                color: "red",
                btns: ["取消", "删除"],
                yes: () => {
                    this.$http
        ...
        save() {
            if (!this.form.workshopName.trim() || !this.form.procedureName.trim()) {
                this.$toast("请输入必填项");
                return;
            }
            this.loading = true;
            let url = this.form.id ? "/haolifa/pay-hour_quota/edit" : "/haolifa/pay-hour_quota/save";
            this.$http
                .post(url, this.form)
                .then((res) => {
                    this.close();
