<template>
    <div class="apply-buy-add">
        <div class="content">
            <icon-btn class="mr-15" @click="$store.commit('DELMENUTABS', '/role/edit');$router.back()">arrow_back</icon-btn>
            ...
            <p>菜单选择</p>
            <div class="flex">
                <el-tree ref="tree" :data="data" :node-click="nodeClick" :default-checked-keys="checkedList" :default-expanded-keys="checkedList" node-key="id" show-checkbox :props="defaultProps"></el-tree>
            </div>
            <div class="flex" style="margin-top:20px;">
                <btn big class="mr-20" @click="submit()">提交</btn>
                <btn big flat @click="$store.commit('DELMENUTABS', '/role/edit');$router.back()">取消</btn>
            </div>
        </div>
        <layer v-if="loading">
        ...
        submit() {
            ...
            this.$http
                ...
                .then(res => {
                    this.loading = false;
                    this.$toast("保存成功");
                    this.$store.commit("DELMENUTABS", "/role/edit");
