<template>
    <div class="material-list">
        ...
        <div class="flex-item scroll-y">
            <data-list ref="list" :page-size="15" :param="filter" url="/haolifa/material-inspect/history/page-list"
                method="get">
                <tr slot="header">
                    <th>报检单号</th>
                    <th>采购合同号</th>
                ...
                <template slot="item" slot-scope="{ item }">
                    ...
                    <td class="t-right">
                        <a href="javascript:;" v-if="item.status == 1" style="margin-right: 3px" class="blue"
                            @click="execStoreRoom(item)">入库</a>
        ...
        <layer v-if="storeRoom.layerShow" :title="'入库'" width="50%">
            <div>
                <div class="flex">
                    <input-box disabled v-model="storeRoom.materialGraphNo" style="width:30%" class="mr-10 ml-20"
                        label="物料图号"></input-box>
                    <input-box disabled v-model="storeRoom.quantity" type="number" style="width:30%" class="mr-10"
                        label="入库数量"></input-box>
                    <input-box v-model="storeRoom.price" type="number" class="mr-10" style="width:30%"
                        label="采购价格"></input-box>
                </div>
                <div class="flex">
                    <input-box disabled v-model="storeRoom.materialBatchNo" style="width:30%" class="mr-10 ml-20"
                        label="批次号"></input-box>
                    <input-box disabled v-model="storeRoom.orderNo" style="width:30%" class="mr-10"
                        label="采购合同号"></input-box>
                    <input-box v-model="storeRoom.supplier" disabled class="mr-10 ml-20" style="width:30%"
                        label="供应商"></input-box>
                </div>
                <div class="flex">
                    <select-box class="ml-20 mr-10" :list="storeRoom.selectStoreRooms" style="width:50%"
                        v-model="storeRoom.roomNo" @change="loadStoreRocks()" label="库房"></select-box>
                    <select-box class="mr-10" :list="storeRoom.storeRoomRacks" style="width:50%" v-model="storeRoom.rackNo"
                        label="库位"></select-box>
                </div>
                <div class="flex"></div>
            </div>
...
export default {
    ...
    methods: {
        ...
        download() {
            ...
            a.setAttribute(
                "href",
                `/haolifa/export/material-entry?startDate=${this.form.startDate
                }&endDate=${this.form.endDate}&entryStatus=${this.form.entryStatus
                }`
            );
            a.click();
