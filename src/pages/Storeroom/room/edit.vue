...
export default {
    ...
    created() {
        ...
        if (id !== undefined && this.$route.name === "room-edit")
            this.getInfo(id);
    },
    methods: {
        getInfo(id) {
            this.$http
                .get(`/haolifa/store-room/getInfo/${id}`)
                .then(res => {
        ...
        cancel() {
            this.$confirm({
                ...
                btns: ["取消", "离开"],
                yes: () => {
                    this.$store.commit("DELMENUTABS", "/room/edit");
        ...
        submit() {
            ...
            this.$http[method](
                `/haolifa/store-room/${form.id ? "update" : "save"}`,
                form
            )
                .then(res => {
                    this.loading = false;
