<template>
  <div class="nuclear-material-nuclear abs scroll-y">
    ...
    <div class="flex mt-20">
      <!-- <el-button type="primary" :loading="loadingFlag" @click="nuclearing()" -->
      <el-button type="primary" @click="nuclearing()">开始核料</el-button>
      <!-- :loading="nuclearCompleteFlag" -->
      <el-button type="primary" @click="nuclearComplete()">核料完成</el-button>
    </div>
        ...
        <div class="flex-item scroll-y ml-20">
          <table class="data-table">
            ...
            <tr v-for="(item, i) in preCheckMaterList">
              ...
              <td>
                {{
                  item.replaceGraphNoList != null &&
                  item.replaceGraphNoList.length > 0
                    ? '是'
                    : ''
                }}
              </td>
...
export default {
  ...
  methods: {
    ...
    nuclearing() {
      ...
      this.$http
        .post(
          `/haolifa/order-product/check-material?orderNo=${this.orderInfo.orderContractNo}`,
          this.nuclearMater
        )
        .then((res) => {
