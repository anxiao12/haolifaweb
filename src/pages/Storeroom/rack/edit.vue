...
export default {
    ...
    created() {
        ...
        if (id !== undefined && this.$route.name === "rack-edit")
            this.getInfo(id);
        this.getRoomList();
    },
    methods: {
        ...
        getInfo(id) {
            this.$http
                .delete(`/haolifa/store-room/rack/getInfo/${id}`)
                .then(res => {
        ...
        cancel() {
            this.$confirm({
                ...
                btns: ["取消", "离开"],
                yes: () => {
                    this.$store.commit("DELMENUTABS", "/rack/edit");
        ...
        submit() {
            ...
            this.$http[method](
                `/haolifa/store-room/rack/${form.id ? "update" : "save"}`,
                form
            )
                .then(res => {
                    this.loading = false;
