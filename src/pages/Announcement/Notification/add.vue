<template>
    <div class="apply-buy-add">
        <div class="content">
            <div class="title b f-18 mb-10">{{form.id ? '编辑' : '新增'}}通知</div>
            <div class="flex">
...
export default {
    ...
    methods: {
        goBack() {
            let url = this.form.id ? "/notification/edit" : "/notification/add";
        ...
        submit() {
            ...
            let url = this.form.id ? "/notification/edit" : "/notification/add";
            if (!this.form.id) {
                this.$http[method](`/haolifa/message/add`, this.form)
                    .then(res => {
                        this.loading = false;
                        this.$store.commit("DELMENUTABS", url);
                        this.$router.push("/notification");
                ...
                this.$http[method](`/haolifa/message/update`, this.form)
                    .then(res => {
                        this.loading = false;
                        this.$store.commit("DELMENUTABS", url);
                        this.$router.push("/notification");
