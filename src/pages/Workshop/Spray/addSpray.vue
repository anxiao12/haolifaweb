<template>
    <div class="apply-buy-add">
        <div class="content">
            ...
            <div class="flex">
                <input-box v-model="form.planner" class="flex-item mr-10" label="计划人"
            ...
            <div class="card flex" style="margin-top: 0;" v-for="(item, i) in form.items" :key="i">
                <div class="flex-item">
                    <div class="flex">
                        ...
                        <select-box :list="classifyNameList" @change="nameChange(i)" v-model="item.materialName"
                            class="flex-item mr-10" label="零件名称"></select-box>
                        ...
                        <el-select class="flex-item mr-10 mycs" v-model="item.materialGraphNo" filterable
                            @change="getBatchNumber(i, item.materialGraphNo)" placeholder="零件图号请选择">
                            <el-option v-for="(obj, j) in item.tuhaoList" :key="j" :label="obj.text"
                    ...
                    <div class="flex">
                        ...
                        <select-box :list="item.batchNumberList" v-model="item.batchNumber" class="flex-item mr-10"
                            label="批次号"></select-box>
                        <input-box v-model="item.material" class="flex-item mr-10" label="材质"></input-box>
                    </div>
                    <div class="flex">
                        <select-box :list="item.sprayColorList" @change="createTuhao(i)" v-model="item.relationNo"
                            class="flex-item mr-10" label="喷涂颜色"></select-box>
                        <input-box v-model="item.number" type="number" class="flex-item mr-10" label="数量"></input-box>
                        <date-picker v-model="item.planCompleteTime" class="flex-item" label="完成时间"
                            style="margin-right: 20px;"></date-picker>
                    </div>
                    <div class="flex">
                        <input-box v-model="item.sprayedGraphNo" type="text" class="flex-item mr-10"
                            label="完成后图号"></input-box>
                    </div>
                    <div class="flex">
...
export default {
    ...
    methods: {
        ...
        getBatchNumber(i, materialGraphNo) {
            this.$http
                .get(`/haolifa/store-room/material-batch-nos?graphNo=${materialGraphNo}`)
                .then((res) => {
        ...
        nameChange(index) {
            ...
            this.classifyNameList.forEach(item => {
                if (this.form.items[index].materialName == item.text) {
                    id = item.id
                }
            })
        ...
        createTuhao(index) {
            ...
            this.form.items[index].sprayedGraphNo = tuhao.slice(0, tuhao.length - this.form.items[index].relationNo.length) + color;
        },
        goBack() {
            if (this.isAdd) {
                this.$store.commit("DELMENUTABS", "/spray/add");
            } else {
                this.$store.commit("DELMENUTABS", "/spray/edit");
            }
        ...
        getSprayColorList() {
            this.$http
                ...
                .then((res) => {
                    this.sprayFtColorList = [];
                    this.sprayFbColorList = [];
                    this.sprayColorList = [];
                    res.map((item) => {
