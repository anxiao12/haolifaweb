    ...
    <div class="page-invoice-list">
        ...
        <div class="flex-v-center tool-bar">
            ...
            $refs.list.update(true)" placeholder="订单号" style="width: 200px;">
                <input type="text" class="flex-item" v-model="filter.demandName" @change="
            $refs.list.update(true)" placeholder="需求方名称" style="width: 200px;">
                <input type="text" class="flex-item" v-model="filter.supplyAgentName" @change="
            $refs.list.update(true)" placeholder="供应方责任人" style="width: 200px;">
                订单状态:
                <select v-model="filter.orderStatus" class="f-14" @change="$refs.list.update(true)">
                    <option value>全部</option>
                    <option v-for="(item,i) in orderStatusList" :key="i" :value="item.value">{{item.text}}</option>
                </select>
            </div>
            <div class="flex-item"></div>
        </div>
        <div class="flex-item scroll-y">
            <data-list ref="list" :page-size="15" :param="filter" url="/haolifa/finance/receivable/list" method="post">
                <tr slot="header">
            ...
            <div class="layer-text" style="padding-bottom: 50px;">
                <div class="form-content metalwork-info">
                    <table class="f-14 order-info">
                        ...
                        <tr>
                            <td colspan="14" class="b">订单状态 : {{ getOrderStatus(info.orderStatus) }}</td>
                        </tr>
                        <tr>
