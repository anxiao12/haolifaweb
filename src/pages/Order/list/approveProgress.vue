<template>
    <div class="abs scroll-y">
        ...
        >
            ...
            >
                <!-- <icon-btn class="mr-15" @click="$store.commit('DELMENUTABS', '/order/approveProgress');$router.back()">arrow_back</icon-btn> -->
...
export default {
    ...
    methods: {
        getInfo(formNo, formId) {
            this.$http
                .get(`/haolifa/flowInstance/flow/progress?formNo=${encodeURIComponent(formNo)}&formId=`)
                .then((res) => {
                    console.log(res);
                    this.list = res;
                })
