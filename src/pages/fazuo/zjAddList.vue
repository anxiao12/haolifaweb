            ...
            <data-list ref="list" :page-size="15" :param="filter" url="/haolifa/valve-seat-entrust/page" method="post">
                ...
                <template slot="item" slot-scope="{ item }">
                    ...
                    <td class="t-right">
                        ...
                        <a href="javascript:;" v-if="item.inspectStatus !=2" style="margin-right: 3px" class="blue" @click="completeMachine(item)">| 质检完成</a>
...
export default {
    ...
    methods: {
        ...
        complete() {
            ...
            this.$http
                ...
                .then(res => {
                    ...
                    this.updateStatus(this.inspectHistoryAdd.valveSeatId, 1);
                })
                .catch(e => {
                    this.loading = false;
                    this.$toast(e.msg || e.message);
        ...
        completeMachine(item) {
            this.$confirm({
                title: "质检完成确认",
                text: `您确定喷涂订单已质检完成？<br>${item.entrustNo}`,
                color: "red",
                btns: ["取消", "确认"],
                yes: () => {
        ...
        updateStatus(id, status) {
            this.$http
                .put(
                    `/haolifa/valve-seat-entrust/inspectStatus/${id}/${status}`
                )
                .then(res => {
                    this.$refs.list.update(true);
                })
